...
from app.config import write_fid_logger
from app.fid.utils.write_fid.match_context import build_write_fid_match_context
...
from app.fid.utils.write_fid.write_takeoff import format_equipment_code_attr_text
...
def _build_takeoff_targets(interfaces: List[dict], dxf_filename: str) -> Tuple[set, set]:
    ...
    for item in interfaces:
        if not parent_line_effective_assigned(item):
            continue
        has_equipment = bool(_normalize_code(item.get("equipmentCode")))
        tee_row = get_tee_off_flag(item) == 1
        if not has_equipment and not tee_row:
            continue
        takeoff_subject = (
...
def _resolve_equipment_code_for_insert(
    ...
    equipment_code = match_equipment_code(
        insert, by_uni_code, dxf_filename, interfaces, match_ctx=match_ctx
    )
    ...
    if takeoff_key and takeoff_key in takeoff_group_map:
        ...
        if block_type in ("VMB_CHEMICAL", "VMB_GASNAME"):
            if not equipment_code and takeoff_branch_map:
                equipment_code = _normalize_code(next(iter(takeoff_branch_map.values()), ""))
...
def apply_takeoff_colors(
    ...
    if match_ctx is None:
        match_ctx = build_write_fid_match_context(interfaces)
    by_uni_code = match_ctx.by_uni_code
    ...
    for insert in takeoff_inserts_all:
        ...
        matched_items = find_matched_assigned_interfaces(
            insert, interfaces, dxf_filename, match_ctx=match_ctx
        )
        matched_item = matched_items[0] if matched_items else None
        tee_highlight = bool(
            matched_item is not None
            and get_tee_off_flag(matched_item) == 1
            and format_equipment_code_attr_text(matched_item)
        )
        if not equipment_code and not tee_highlight:
            continue
        if _is_takeoff_insert(insert, interface_code, id_code, takeoff_code_keys, takeoff_block_ids):
            assigned_inserts.append(insert)
            continue
        if tee_highlight:
            ic = _normalize_code(interface_code)
            uc = _normalize_code(matched_item.get("uniCode"))
            co = _normalize_code(matched_item.get("code"))
...
def write_equipment_code(
    ...
    write_fid_logger.info(
        f"FID回填: 开始写入设备编码, INSERT总数={len(inserts)}, "
        f"接口数={len(interfaces)}, 已派点={assigned_count}"
    )
    ...
    for insert in inserts:
        ...
        wrote_for_insert = dispatch_write_equipment_by_block_type(
            ...
            matched_items=matched_items if not is_takeoff_block else None,
        )
