...
from app.fid.utils.process_fid_request import (
...
async def _fid_check(
    ...
    try:
        ...
        for il in interface_list:
            snake_il = normalize_interface_record(convert_dict_keys_to_snake(il))
            if il['status'] != 'not_existing_in_fid':
                active_interface_list.append(snake_il)
            else:
                delete_interface_list.append(snake_il)

        field_list = [convert_dict_keys_to_snake(fl) for fl in field_list]
        interface_list = normalize_interface_list(
            [convert_dict_keys_to_snake(il) for il in interface_list]
        )
