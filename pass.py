...
# FTP 上回填结果完整相对路径前缀（与服务器目录 /EFMS/fid_with_assignment 对应）
...
def clear_equ_attributes(doc: ezdxf.document.Drawing) -> int:
    """清空图纸内所有 EQU* 属性文本，仅保留属性结构。"""
    cleared = 0
    msp = doc.modelspace()
    for insert in msp.query("INSERT"):
        for attrib in getattr(insert, "attribs", []):
            tag = str(getattr(attrib.dxf, "tag", "") or "").upper()
            if tag.startswith("EQU"):
...
def build_write_fid_ftp_result_names(source_stem: str) -> Tuple[str, str]:
    """
    回填结果文件名及 FTP/API 返回用的远端相对路径：
    返回 (out_filename, rel_path)，rel_path 形如 EFMS/fid_with_assignment/{stem}_{时间戳}.dxf
    """
    ts = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
    out_name = f"{source_stem}_{ts}.dxf"
    rel_path = f"{WRITE_FID_FTP_REMOTE_PREFIX}/{out_name}"
