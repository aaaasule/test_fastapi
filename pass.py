...
def normalize_interface_record(data: dict) -> dict:
    """
    标准化单条 interface 记录：保证属性对比所需字段始终存在。

    请求里部分条目带 inOutCode、部分不带时，pandas 仍会有列，但缺失行是 NaN；
    对比前统一为 str，缺省为 ''，避免 lookup 取值不一致。
    """
    if not isinstance(data, dict):
        return data
    record = dict(data)
    raw = record.get('in_out_code')
    if raw is None or (isinstance(raw, float) and raw != raw):  # NaN
        record['in_out_code'] = ''
    else:
        record['in_out_code'] = str(raw).strip()
    return record


def normalize_interface_list(items: list) -> list:
    """批量标准化 interface_list（元素须已为 snake_case）。"""
    return [normalize_interface_record(item) for item in items]
